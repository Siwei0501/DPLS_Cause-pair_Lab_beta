...
import pandas as pd
import sys
# 获取 .exe 所在的真实路径（而不是临时解包路径）

base_dir = os.path.dirname(os.path.abspath(__file__))
airfoil_file = pd.read_csv(rf'{base_dir}/airfoil_self_noise.dat', sep='\s+', header=None)
...
if __name__ == '__main__':
    return_cause_pair(need_y=True, keep_col_names=True)