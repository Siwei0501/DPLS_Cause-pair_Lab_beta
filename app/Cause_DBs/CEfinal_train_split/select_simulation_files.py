...
def select(relation: Literal['AB', 'BA'], threshold: list, linear: bool = False, test_SAMPLE: int = 200, seed=5):
    np.random.seed(seed)
    random.seed(seed)
    # 文件路径
    if linear:
        folder_path = rf'../Cause_effect_pair\simulation\dataset\Linears\{relation[0]}→{relation[1]}'
    else:
        folder_path = rf'../Cause_effect_pair\simulation\dataset\Non_linears\{relation[0]}→{relation[1]}'

    if os.path.exists(rf'simulation/temp_file'):
        pass
    else:
        os.mkdir(rf'simulation/temp_file')
        print('已创建临时文件夹 temp_file')

    # 获取当前目录下所有以 'train' 开头并以 '.txt' 结尾的文件
    files = [f for f in os.listdir(folder_path) if f.endswith('.csv')]
    # 定义用于记录处于阈值内文件的文件名
    record_file = rf'simulation\temp_file\{relation}_files_OUT_of_{threshold[0]}-{threshold[1]}.txt'
    print("Reading..")
    ...
    read_in_files = [pd.read_csv(os.path.join(folder_path, file), sep=',', header=None) for file in
                     selected_files]
    file_names = ['y=' + name_[:-4] for name_ in selected_files]

    return read_in_files, file_names


if __name__ == '__main__':
    select('AB', threshold=[400, 600], linear=True, test_SAMPLE=20)
