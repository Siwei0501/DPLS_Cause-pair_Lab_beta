...
def return_cause_pair(relation: Literal['AB', 'BA', 'AB&BA'], threshold: list, test_SAMPLE: int = 200, seed=5, **kwargs):
    ...
    for relation_ in relation:
        ...
        files = [f for f in os.listdir(folder_path) if f.startswith('pair') and f.endswith('.txt')]
        # 定义用于记录处于阈值内文件的文件名
...
if __name__ == '__main__':
    ...
    read_files, file_names, file_ys, X, y = return_cause_pair(relation='AB&BA', threshold=[0,10000], test_SAMPLE=50, seed=5)
    a=1