...
def create_data(block_id, thread=1):
    ...
    with create_preview_panel:
        ...
        formular_clear_col, formular_sep_1, formular_title_add_x, add_x_col, minus_x_col,  formular_sep_2, formular_title_add_xtox, add_xtox_col, minus_xtox_col, formular_sep3, formular_refresh_col  = st.columns([.6, .3, .75, .21, .21, .3, .75, .21, .21, 1.2, .9])
    
        # with formular_title_col:
        #     render_section_title("你的函数 be like :", underline=False)
    
        with formular_clear_col:
        ...
        with formular_refresh_col:
            ...
            refresh_button = st.button('⇆', key=f"{block_id}_refresh_button", use_container_width=True)
