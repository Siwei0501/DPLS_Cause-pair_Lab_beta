...
from dpls_scripts.DPLS_jj import DPLS
...
def cal_X_e_P_HSIC(file_value: pd.DataFrame, P_mode: Literal['min', 'max', 'mean'] = 'mean', **kwargs):
    ...
    if np.any(np.isnan(reason_result_obj.R2[0])) or np.any(np.isnan(result_reason_obj.R2[0])):
        return 0, 0
    ...
    if P_mode == 'min':
        ...
        raise ValueError('cal_X_e_P_HSIC: P_mode must be one of "min", "max", "mean"')
...
def cal_X_e_KCI(DPLS_obj: DPLS, P: None | int = None, **kwargs):
    ...
    if np.any(np.isnan(DPLS_obj.R2[0])):
        return 0
...
def cal_P_DPLSR(file_value: pd.DataFrame, P_mode: Literal['min', 'max', 'mean'] = 'mean', **kwargs):
    ...
    if P_mode == 'min':
        ...
        raise ValueError('cal_P_DPLSR: P_mode must be one of "min", "max", "mean"')
...
def cal_break_DPLSR(file_value: pd.DataFrame, break_parts=2, **kwargs):
    ...
    for part_i in range(break_parts):
        idx_i = value_sorted[(value_sorted[kwargs['reason']] > reason_min + break_point * (part_i)) & (
                    value_sorted[kwargs['reason']] <= reason_min + break_point * (part_i + 1))].index

        idx_obj = DPLS(**kwargs).fit(value_sorted.loc[idx_i, kwargs['reason']],
                                     value_sorted.loc[idx_i, kwargs['result']], **kwargs)
...
def cal_meanX(file_value: pd.DataFrame, mean_ratio=0.2, Xmean_mode: Literal['uniform', 'normal'] = 'normal', Xmean_window=2,
    ...
    if Xmean_mode == 'uniform':
        ...
        raise AttributeError(f'cal_meanX: 不存在的Xmean_mode: {Xmean_mode}')
...
def cal_path(file_value: pd.DataFrame, Path_centre: bool = False, Path_normal: bool = False, Path_window: int = 1,
             Path_abs=True, Sort_by: Literal['reason', 'all'] = 'all', **kwargs):
    value_copy = file_value.copy()
    value_copy = value_copy.dropna(axis=0, how='any')
...
def cal_shuffle_path(file_value: pd.DataFrame, Path_centre: bool = False, Path_normal: bool = False,
                     Path_window: int = 1, Path_abs=True, Shuffle_times=1000, **kwargs):
    value_copy = file_value.copy()
    ...
    for i in range(Shuffle_times):
        ...
        reason_value, result_value = cal_path(file_value=value_shuffled, Path_centre=Path_centre,
                                              Path_normal=Path_normal, Path_window=Path_window, **kwargs)
...
def cal_path_tender(file_value: pd.DataFrame, Tender_length: int = 5, Path_centre: bool = False,
                    Path_normal: bool = False, **kwargs):
    value_copy = file_value.copy()
...
def add_noise(file_value: pd.DataFrame, data_R=None,
    ...
    if adjust:
        if data_R is not None:
            ...
            use_level = DPLS(**kwargs).fit(data_copy[kwargs['reason']], data_copy[kwargs['result']], **kwargs).R2[
                            0] * noise_level
...
def return_name(method, reverse=False, **kwargs) -> list[str]:
    ...
    if method == 'DPLS':
        ...
        if reverse:
            col_name = [f"KCI_p_{kwargs['pre_process']}(B, A_e)"]
        else:
            col_name = [f"KCI_p_{kwargs['pre_process']}(A, B_e)"]
        ...
        if reverse:
            col_name = [f"BA-DPLSe_KCI-p-{P_mode}{kwargs['pre_process']}(B, A)",
                        f"BA-DPLSe_KCI-p-{P_mode}{kwargs['pre_process']}(A, B)"]
        else:
            col_name = [f"AB-DPLSe_KCI-p-{P_mode}{kwargs['pre_process']}(A, B)",
                        f"AB-DPLSe_KCI-p-{P_mode}{kwargs['pre_process']}(B, A)"]
        ...
        if reverse:
            col_name = [f"KCI_p_{kwargs['pre_process']}(B&CMV, A_e)"]
        else:
            col_name = [f"KCI_p_{kwargs['pre_process']}(A&CMV, B_e)"]

    elif method == 'CMVe_DPLSe_KCI':

        if reverse:
            col_name = [f"KCI_p_{kwargs['pre_process']}(B_e, A_ee)"]
        else:
            col_name = [f"KCI_p_{kwargs['pre_process']}(A_e, B_ee)"]
        ...
        if reverse:
            col_name = [f"PATH_{Tender_length_i + 1}_{kwargs['pre_process']}(B, A)" for Tender_length_i in
                        range(Tender_length)]
        else:
            col_name = [f"PATH_{Tender_length_i + 1}_{kwargs['pre_process']}(A, B)" for Tender_length_i in
                        range(Tender_length)]
...
def return_processed_files(file_value_list: list[DataFrame], pre_process: Pre_process_Iterable = '', thread=1,
                           **kwargs):
...
def return_values(file_value_list: list[DataFrame],
    ...
    if method in algorithms.keys():
        values_result = parallel_wrapper(func=algorithms[method], file_value_list=used_value_list, desc=method,
...
def return_Cause_DF(file_value_list: list[DataFrame], pre_process: Pre_process_Iterable = '',
                    method: Method_Option = '', reverse=False, thread=1, **kwargs):
    # 默认第一列为原因
    if reverse != 1:
        reason = 0
        result = 1
    ...
    return_results = return_values(file_value_list=used_value_list, method=method, thread=thread, reason=reason,
                                   result=result, **kwargs)

    return_DF = return_values_DF(return_results, pre_process=pre_process, method=method, reverse=reverse, thread=thread,
                                 **kwargs)
